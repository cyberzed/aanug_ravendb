...
{
	var hans = new Speaker("Hans", "@snydertamp");
	
	documentSession.Store(hans);
...
using (var documentSession = documentStore.OpenSession())
{
	var hans = documentSession.Query<Speaker,SpeakerNameIndex>()
					.Search(sp=>sp.Name, "hans")
					.Single();
					
	hans.Dump();
					
	documentSession.Delete(hans);
